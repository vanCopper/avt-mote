package   UISuit.UIController 
{
	...
	public class BSSDataGrid extends Sprite
	{
		...
		{    
		    if (!m_boardTypeArray || !(m_boardTypeArray[i] & BSSUIDEF.BSSDATAGRID_NOT_SORT))
		        btn.activate();
		    i++;
		}
		...
		: void {
			...
			for (var i : int = startPos ; i < endIndex ; ++i , ++j )
			{
				...
				if (boardTypeArray)
				{
					...
					if ((m_boardTypeArray[i] & BSSUIDEF.BSSDATAGRID_NOT_SORT) && m_boardButtonArray[i])
		...
		: BSSDataGrid {
			...
			var tabulation : BSSTabulation = BSSTabulation.createSimpleBSSTabulation(20,h - 18,selectble);
			tabulation.y = 18;
