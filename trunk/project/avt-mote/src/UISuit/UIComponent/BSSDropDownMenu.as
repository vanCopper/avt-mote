package   UISuit.UIComponent   {  
	...
	public class BSSDropDownMenu  extends  Sprite  { 
		...
		protected var m_scrollItemArray : Array = new Array();
		...
		: void {
			...
			if (m_scrollContainer) { GraphicsUtil.removeAllChildren(m_scrollContainer); m_scrollContainer = null; }
			m_scrollBG  = null;
			m_scrollItem  = null;
		...
		_areaArray : Array  = null 
		) : BSSDropDownMenu
		{
			var _allMenuBg : Shape = new Shape();
			GraphicsUtil.DrawRect( _allMenuBg.graphics , 0 , 0 , w, h , 0xFFFFFF , 1 , 1 ,0x0 ,1  );
			...
			var _hitBtn : BSSButton ;
			{
				...
				GraphicsUtil.DrawRect( shape.graphics , 0 , 0 , btnW, h - (edge<<1) , 0xEEEEFF , 0.5 , 0 ,0x0 ,0.2  );
				doc.addChild(shape);

				shape = new Shape();
				GraphicsUtil.DrawRect( shape.graphics , 0 , 0 , btnW, h - (edge<<1) , 0xEEEEFF , 0.9 , 0 ,0x0 ,0.5  );
				doc.addChild (shape);

				shape = new Shape();
				GraphicsUtil.DrawRect( shape.graphics , 0 , 0 , btnW, h - (edge<<1) , 0xCCCCFF , 1 ,  0 , 0xFFFFFF ,0.5  );
				doc.addChild (shape);
			...
			GraphicsUtil.DrawRect( _scrollItem.graphics , 0 , 0 , w, (int)(int(ddm.m_itemTemplateTextFormat.size) + 6) , 0x4444FF , 0.5 );

			return ddm;
		}  
		
		public function BSSDropDownMenu( _hitBtn : BSSButton ,  _allMenuBg : DisplayObject , _hintText : TextField , _scrollBG : DisplayObject  
		, _scrollItem : DisplayObject   , hintString : String = null , _arerArray : Array = null
...
//					ASSERT(m_scrollItem.visible , ("error "));
		...
		: void {
			...
			if (m_hintText.text == getSelectedString())
			{
				m_hintText.text = str;
...
*/
		...
		public function setSelectedString( str : String , callBack : Boolean = false) 
		: void {
			var i : int = 0;
