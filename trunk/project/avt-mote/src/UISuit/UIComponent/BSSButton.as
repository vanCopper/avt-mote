package   UISuit.UIComponent   {  
 ...
 ;  	
	...
	public class BSSButton  extends  Sprite  { 
		...
		protected  var areaArray : Array ; 
		
		public function setAreaArray(a_areaArray : Array) : void
		{
			CONFIG::ASSERT
			{
				ASSERT(areaArray == null && a_areaArray != null && a_areaArray.indexOf(this) != -1 , "error set areaArray");
			}
			areaArray = a_areaArray;
		}
		...
		//#ifdef USE_MOVE_ON_FUNC
		public var moveOnFunction : Function ;
		//#endif
		//#ifdef USE_MOVE_OUT_FUNC
	...
	*/
		...
		: BSSButton {
			...
			GraphicsUtil.DrawRect( shape.graphics , 0 , 0 , w, h , 0xFFFFFF , 0.7 , 0 ,0 ,0.1  );
			doc.addChild (shape);

			shape = new Shape();
			GraphicsUtil.DrawRect( shape.graphics , 0 , 0 , w, h , 0xFFFFFF , 0.9 , 0 ,0 ,0.5  );
			doc.addChild (shape);

			shape = new Shape();
			GraphicsUtil.DrawRect( shape.graphics , 0 , 0 , w, h , 0x62B0FF , 1 ,  0 , 0xFFFFFF ,0.5  );
			doc.addChild (shape);
		...
		: BSSButton {
			...
			GraphicsUtil.DrawRect( shape.graphics , 0 , 0 , w, h , 0x0000FF , 0  );
			CONFIG::DEBUG {
				GraphicsUtil.DrawRect( shape.graphics , 0 , 0 , w, h , 0x0000FF , 0.1  );
			}
			doc.addChild (shape);
			doc.x = x;
			doc.y = y;
		...
		public function BSSButton (data : DisplayObjectContainer , hitString : String = null, autoWidth : Boolean = false , _areaArray : Array  = null 
...
//           DBG_TRACE("state "+ state);
			...
			if (buttonMode && moveOnFunction!= null && state != BSSButton.SBST_IDLE_OVER )
				moveOnFunction(this);
			//#endif
		...
		: void {
			...
			if (state == BSSButton.SBST_PRESS)
			{
				...
				{
					if (releaseFunction.length == 1)
						releaseFunction(this);
					else
						releaseFunction(this  , e);
				}
			}
		}
		...
		public  function onMousePress(e : MouseEvent)
		 : void {
			...
			//#ifdef MOVE_ON_FUNCTION}
			//#ifdef USE_MOVE_ON_FUNC
				if (state ==  BSSButton.SBST_IDLE )
				{
					if (moveOnFunction != null)
						moveOnFunction(this);
				}
			//#endif
