package   UISuit.UIComponent   {  
	...
	//import  UISuit.Main.*;
	//import UISuit.Engine.*;

	//import UISuit.Core.*;
	//import UISuit.Spec.*;
	//import UISuit.Debugger.*;
	...
	public class BSSScrollBar extends Sprite 
	{
		...
		: BSSScrollBar {
			...
			GraphicsUtil.DrawRect( bgShape.graphics ,  0 , 0 , w , w<<2 , 0xF5F5F5 , 0.75 );
			...
			for (var i : int = 0; i < shapeNum ; i++ )
			{
				...
				{	
					shape = new Shape();
					GraphicsUtil.DrawRect( shape.graphics , 0 , 0 , w, w , (i==0 ? -0x0606 : 0 ) + ((j == 2) ? 0xFF3333  :  ((j == 1) ? 0xFFDDDD : 0xFFFFFF )) , 0.5 + j * 0.1 , 0 , 0 ,0.15  );
...
//#ifdef ENABLE_SB_WHEEL
            ...
            : void {
                 if (s_BSSScrollBar_WheelFocusBSSScrollBar == fBSSScrollBar_WheelFocusBSSScrollBar )  
		...
		public function BSSScrollBar(data : DisplayObjectContainer , minContent : Number = 0 , maxContent : Number = NaN , specEdgeOff : Number = 0)
		{
			...
			if (data.numChildren > 2)
			{
					...
					m_scrollUpButton.releaseFunction = m_scrollUpButton.moveOutFunction = scrollUpButton_MouseUp; //MOUSE_UP
					...
					m_scrollDownButton.releaseFunction = m_scrollDownButton.moveOutFunction = scrollDownButton_MouseUp;	
		...
		public  function  scrollDownButton_MouseUp(btn : BSSButton = null)	: void
		{
			...
			CONFIG::ASSERT {
				if (m_scrollBar_moveFlag == 1 || m_scrollBar_moveFlag == 0 )
				{
